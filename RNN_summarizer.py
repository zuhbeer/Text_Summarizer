...
#saves model, a csv with a summarized column'''

#set wordcount threshold for training, different datasets will recquire diff thresholds
...
def Model_building(x_voc, y_voc):
    ...
    print(model.summary())
    return model, decoder_inputs, dec_emb_layer, decoder_lstm, decoder_dense, attn_layer
...
def main(filename):
    df = clipper(filename)
    x_tr,x_val,y_tr,y_val,x_voc,y_voc,x_tokenizer,y_tokenizer = split_process(df)
    model, decoder_inputs, dec_emb_layer, decoder_lstm, decoder_dense, attn_layer = Model_building(x_voc, y_voc)
    compiler(x_tr,x_val,y_tr,y_val)
...
def decode_sequence(input_seq, encoder_model, decoder_model, reverse_target_word_index, target_word_index):
...
def summary_df(reverse_source_word_index, target_word_index, reverse_target_word_index, entries = 10000):
    rev = []
    tru = []
    summ = []
    for i in range(entries):
        rev.append(seq2text(x_tr[i], reverse_source_word_index))
        tru.append(seq2summary(y_tr[i],target_word_index, reverse_target_word_index))
...
if __name__ == '__main__':
    if os.path.isfile('training.csv'):
        ...
        encoder_model, decoder_model, reverse_target_word_index, reverse_source_word_index, target_word_index = main(data)
        summary_df(reverse_source_word_index, target_word_index, reverse_target_word_index)
        x,y = BLEU_score.lists('Summarized.csv')
        Blue_score.bleu_score(x,y)
    else:
        preprocessor.preprocess('data/Reviews.csv')
        data = pd.read_csv('training.csv')
        main(data)
        summary_df()
        x,y = BLEU_score.lists('Summarized.csv')
